...
// }else{
	submitUrlBase = "http://atwlab-atwlab.rhcloud.com";
// }
...
var _submitOnServer = function(isUnfinished, _sessionId, objToSubmit, logsToSubmit, callback) {
	var sessionId = (require("./resources/_settings.js")).getCondition() + "_" + _sessionId;
		...
		self.addObj = function(outpName, obj) {
			...
			if(obj && obj.length){
				for(var i = 0, ilen = obj.length; i < ilen; i++){
					_storage[outpName].push(obj[i]);
				}
			}else{
				_storage[outpName].push(obj);
			}
			return _storage[outpName].length;
		};
