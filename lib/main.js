...
//var MainTabProtector =
// require("./tracker.main.tab.protector.js").MainTabProtector;
...
function _init() {
	...
	allModules["nextStep1"] = new NextStepMessageModule(props, "Rate (single) webpage aesthetics");
	allModules["nextStep2"] = new NextStepMessageModule(props, "Read a scenario, visit websites, order the website list");
	allModules["nextStep3"] = new NextStepMessageModule(props, "Search info on websites; Rate the websites and companies");
	allModules["nextStep4"] = new NextStepMessageModule(props, "Last effort. Free-text website description.");
...
function attachGlobalEventHandlers(eventBroadcaster, allModules, mainTab) {
	eventBroadcaster.on("test.flow.interrupted", function() {
		...
		//handelTestFinish(allModules["endMessage"], mainTab);
	});
...
function getOnCloseEmergencySave() {
	...
	return function() {
		if(props.debug) {
			return console.log("EmergencySave has been cancelled since we are in a debug mode");
		}
		if(++counter < 3) {
			if(!props.isTestDone) {
...
function emergencyFlush(allModules, lastModuleName, mainTab){
	for(var _keys = Object.keys(allModules), i = _keys.length; i--; ){
		if(_keys[i] != lastModulesName){
			allModules[_keys[i]].emitDone();
		}
	}
	return handelTestFinish(allModules[lastModuleName], mainTab);
}

function handelTestFinish(endModule, mainTab) {
	if(endModule){
		...
		lastPage.saveAllExperimentalData();		
	}else{
											...
											allModules["nextStep4"].createPage(function() {
												...
												allModules[_websiteDescriptModuleNames[0]].createPage(function() {
													allModules[_websiteDescriptModuleNames[1]].createPage(function() {
														return handelTestFinish(allModules["endMessage"], mainTab);
														// THE END OF LOGIC
