...
var LookingTimeTracker = function(props, moduleName, trackInDetailWebs, exclude) {
	...
	function handleOnActivate(tab) {
		console.log("Activated tab: " + tab.url);
		checkAndSave(tab);
	};

	var checkAndSave = function(tab) {
		if(trackedWebpage.isTracked()) {
			...
			if(checkPositionWorker) {
				// if everything as planned, i.e., if the tab exists and the associated page
				// isn't loading/unloading.
	...
	inits.onAttach = function(worker, storage, console, self) {
		...
		worker.port.on("is.top.changed", function(data) {
			...
			if(!trackedWebpage.isTracked()) {
				throw new Error("The url of this tab (" + data.url + ") is valid and should be tracked, but it's not tracked");
			}
