...
function relativeToFullUrl(urlArr){
	for(var i = 0, ilen = urlArr.length; i < ilen; i++){
		urlArr[i] = self.data.url(urlArr[i]);
	}
	return urlArr;
}
scriptsToLoad = relativeToFullUrl(scriptsToLoad);
cssToLoad = relativeToFullUrl(cssToLoad);
