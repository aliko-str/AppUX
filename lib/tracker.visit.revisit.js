...
var BrowsingTracker = function(props, moduleName, includePtrn, trackInDetailWebs, exclude) {
	var inits = {
	...
	inits.outpManager = constructOutpObj(inits.moduleName, trackInDetailWebs);
	...
	inits.onAttach = function(worker, storage, console, self) {
		worker.port.on("history.visit", function(data) {
			...
			var outVisitName = self._getOutpNameByHostname("visit", data.baseUrl);
			if(outVisitName) {
				...
				var outReferName = self._getOutpNameByHostname("visit", data.referrerBaseUrl);
